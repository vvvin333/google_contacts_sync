...
# The user field that we store a key in to
# uniquely identify a person across accounts
SYNC_TAG = "csync-uid"

# all personFields, I don't know to programmatically get these,
# we just got them from
...
class Contacts:
    ...
    def __init__(self, keyfile, credfile, user, verbose=False):
        creds = None

        # The file token.json (name is taken from config.ini "credfile")
    ...
    def __strip_body(self, body):
        ...
        # for some reason some of my contacts have more than one name
        # remove anything except the first
        # same problem with genders and birthdays
    ...
    def fetch_info(self):
        ...
        """
        for p in self.fetch_all_contacts():
            tagls = [
                kv["value"]
    ...
    def update_tag(self, rn: str, tag: str):
        ...
        tag: str
            The tag to add.
            TODO: No check on uniques is made, but it better be
    ...
    def delete_contactGroup(self, tag: str, verbose=False):
        # need to find the resource name
        rn = self.tag_to_rn_contactGroup(tag)
        if rn is None:
            return
