...
editing your config file (you will be prompted about that), you should be all
good to go.
...
if args.init:
    ...
    for email, acc in contacts.items():
    ...
    for email, acc in contacts.items():
        ...
        for rn, p in acc.info.items():
            if p["name"] in done:
                ...
                for otheremail, otheracc in contacts.items():
                    if otheracc == acc:
                        continue
...
for email, acc in contacts.items():
    ...
    for rn, name in toadd:
        ...
        for otheremail, other in contacts.items():
            if other == acc:
                continue
